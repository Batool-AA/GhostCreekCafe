...
void Salad::move(waiter_class obj)
{
...
void Salad::set_mover(int x, int y, int w, int h){
    moverRect.x = x;
    moverRect.y = y;
    moverRect.w = w;
    moverRect.h = h;
...
int Salad::check(vector<Menu*> countorder) //checks if there is an empty counter to display food
{
...
Salad::Salad(SDL_Rect scr, SDL_Rect move){ //constructor defining the srcrect and moverect
    srcRect = scr; 
    moverRect = move; 
}